...
function activitiesPage() {
  ...
  const titles = { home: 'Little adventures.<br>Closer connections.', events: 'A little plan. A great day out.', saved: 'Good finds, kept for later.' };
  return `${home ? '<div class="hero-shell">' : ''}<section class="home-intro"><div class="eyebrow">${savedPage ? 'YOUR COLLECTION' : 'BRISBANE / FAMILY ACTIVITIES'}</div><h1>${titles[route]}</h1><p class="intro">${savedPage ? 'Your next family day out, all in one place. Saved to your account for whenever you’re ready.' : 'Discover free activities, local markets and things to do together — with a little less spending.'}</p>${home ? '<div class="hero-actions"><a class="button primary" href="#events">Find an activity</a><a class="button" href="#community">Meet your community</a></div>' : ''}</section>${home ? '<div class="hero-picture" id="home-hero-image"><span class="image-fallback">' + icon('leaf',45) + '</span></div></div>' : ''}
